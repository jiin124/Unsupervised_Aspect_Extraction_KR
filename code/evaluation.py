...
parser.add_argument("-o", "--out-dir", dest="out_dir_path", type=str, metavar='<str>', required=True, help="The path to the output directory")#출력 디렉토리 경로
...
parser.add_argument("--domain", dest="domain", type=str, metavar='<str>', default='hotel', help="domain of the corpus hotel")
...
# #평가 후 #으로 다시 하기 
#위의 코드는 out_dir를 코드 구현자가 pre_trained 된 데이터로 테스트 할 수 있게 제공. 

U.print_args(args)

assert args.algorithm in {'rmsprop', 'sgd', 'adagrad', 'adadelta', 'adam', 'adamax'}
assert args.domain in {'hotel','restaurant'}#'restaurant', 'beer'
...
def evaluation(true, predict, domain):
    ...
    if domain == 'hotel':
        ...
        with open(out_dir + '/predict_label', 'w', encoding='utf-8') as f:
...
def prediction(test_labels, aspect_probs, cluster_map, domain):
    ...
    evaluation(open(test_labels,'rt', encoding='UTF8'), predict_labels, domain)
...
#input 뒤에 ,K.learning_phase() 삭제
#att_weights, aspect_probs = test_fn([test_x, 0])
att_weights, aspect_probs = test_fn([test_x])#위의 코드 수정
