...
Codes and Dataset for ACL2017 paper ‘‘An unsupervised neural attention model for aspect extraction’’. [(pdf)](http://aclweb.org/anthology/P/P17/P17-1036.pdf) 를 최신버전으로 수정한 것입니다. 
이 레포지토리는 paper authors' repository에서 코드 개선을 한 코드가 포함되어있습니다. 
기존에는 theano를 사용하였지만 이 코드는 tensorflow를 사용합니다. 
...
```
python train.py --emb "../preprocessed_data/$domain/w2v_embedding" --domain "$domain" -o "output_dir"
```
...
```
python evaluation.py --domain "$domain" -o "output_dir" 
```
